...
diary_gold_source_id = [53, 109, 144, 200, 203, 230, 255, 256, 317, 368, 398, 413, 421, 464, 525, 533, 558, 562, 572, 573, 576, 638, 657, 712, 730, 747, 750, 765, 811, 931, 936, 968, 990, 1029, 1035, 1190, 1229, 1280, 1295, 1380, 1449, 1465, 1475, 1518, 1586, 1596, 1643, 1653, 1701, 1740, 1801,
                        1879, 1922, 1943, 1983, 2044, 2087, 2108, 2129, 2132, 2155, 2260, 2334, 2360, 2407, 2424, 2446, 2461, 2510, 2551, 2648, 2670, 2689, 2746, 2883, 2964, 2965, 2988, 2997, 3004, 3083, 3188, 3342, 3409, 3439, 3454, 3527, 3694, 3815, 3913, 3973, 4399, 4444, 4478, 4489, 4547, 4566, 4609, 4662, 4664]
tweet_gold_source_id = [5, 71, 78, 168, 190, 291, 421, 550, 590, 728, 930, 960, 1221, 1255, 1267, 1321, 1383, 1713, 1791, 1869, 1998, 2055, 2676, 2717, 2817, 3723, 3888, 4154, 4209, 4214, 4355, 4398, 4559, 4601, 4620, 4675, 4708, 4755, 5851, 6292, 7116, 7208, 7265, 7353, 7381, 8426, 9864, 10857, 11226, 11298, 12034, 12066, 12192,
                        13005, 13409, 13613, 14576, 15747, 15823, 16736, 16766, 17222, 17475, 18426, 18825, 20534, 20821, 20982, 21167, 22721, 22749, 22992, 23200, 23318, 23401, 23877, 24075, 24212, 25046, 25517, 25576, 25688, 26099, 26718, 27282, 27313, 27330, 27451, 27517, 27648, 27728, 28007, 28257, 28581, 28791, 28798, 29177, 29512, 29674, 30145]
...
def get_entries(source_type):
    ...
    normal_entries = Entry.objects.filter(
        source_type=source_type, golden=False)
...
if __name__ == '__main__':
    ...
    assert(Entry.objects.filter(
        source_type=Entry.TWEET, golden=True).count() == 100)
    assert(Entry.objects.filter(
        source_type=Entry.DIARY, golden=True).count() == 100)
    ...
    with open('accounts.txt', 'w') as f:
        ...
        for i in range(golden_accounts):
            ...
            diary_annotator_golden = User.objects.create_user(
                username, '', password)
        ...
        for i in range(diary_accounts):
            ...
            put_entries(diary_normal[i::diary_accounts] +
                        list(diary_golden), annotator)
            write_account_info(username, password, f)
        ...
        for i in range(golden_accounts):
            ...
            password = random_password()
            tweet_annotator_golden = User.objects.create_user(
                username, '', password)
        ...
        tweet_normal_split12 = []
        for i in range(12):
            tweet_normal_split12.append(tweet_normal[i::12])

        tweet_normal_split = []
        for i in range(5):
            tweet_normal_split.append(
                tweet_normal_split12[i * 2] + tweet_normal_split12[i * 2 + 1])
        tweet_normal_split.append(tweet_normal_split12[-2])
        tweet_normal_split.append(tweet_normal_split12[-1])

        for i in range(tweet_accounts):
            username = 'tweet_annotator00%d' % (i + 1)
            password = random_password()
            annotator = User.objects.create_user(username, '', password)
            put_entries(tweet_normal_split[i] + list(tweet_golden), annotator)
            write_account_info(username, password, f)
